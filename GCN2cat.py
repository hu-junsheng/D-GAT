...
class GATNet(nn.Module):
    ...
    def forward(self, data_hrtf, data_pos):
        ...
        # `batch_data.batch` 是一个包含图的批次信息的张量
        ...
        # Global Pooling
        # x_hrtf = global_mean_pool(x_hrtf, data_hrtf.batch)
        # x_pos = global_mean_pool(x_pos, data_pos.batch)
        # x_weight = torch.cat([x_hrtf, x_pos], dim=-1)
        # x_pos_last = x_pos_last.unsqueeze(1)
        # x_pos_last = self.conv1(x_pos_last)
        # x = torch.bmm(x_hrtf_last.unsqueeze(1),x_pos_last)
        ...
        # x = torch.cat([x_hrtf, x_pos], dim=-1)
        # x = x_hrtf * x_pos
